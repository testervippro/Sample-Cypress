...
Cypress.Commands.add("selectProduct", (productName: any) => {
  cy.get("h4.card-title").each(($el, index, $list) => {
    if ($el.text().includes(productName)) {
      cy.get(".zmdi.zmdi-shopping-cart").eq(index).click();
      //cy.get('.btn.btn-info')
    }
  });
});
...
Cypress.Commands.add("getMicrosoftDenfenderPolicy", (setScriptPolicyPath) => {
  return cy
    .exec(
      `powershell -File "cypress\\defender\\setPolicy.ps1" -username "ad.onmicrosoft.com" -password "Demo" -command ${setScriptPolicyPath}`,
...
Cypress.Commands.add("setMicrosoftDenfenderPolicy", (setScriptPolicyPath) => {
  const _sciptSetPolicy = "cypressdefendersetPolicy.ps1";
  return cy
    .exec(
      `powershell -File cypress\\defender\\setPolicy.ps1 -username "lukecom" -password "Demo" -command ${setScriptPolicyPath}`,
      { failOnNonZeroExit: false }
    )
    .then((result) => {
      return {
